...
main() {
  ...
  RATE=$(get_tmux_option "@penumbra-refresh-rate" 5)
  cpu_load=$(get_tmux_option "@penumbra-cpu-display-load" false)
  if [ "$cpu_load" = true ]; then
    echo "$(get_load)"
  else
    cpu_label=$(get_tmux_option "@penumbra-cpu-usage-label" "CPU")

...
IFS=' ' read -r -a hide_status <<< $(get_tmux_option "@penumbra-git-disable-status" "false")
IFS=' ' read -r -a current_symbol <<< $(get_tmux_option "@penumbra-git-show-current-symbol" "✓")
IFS=' ' read -r -a diff_symbol <<< $(get_tmux_option "@penumbra-git-show-diff-symbol" "!")
IFS=' ' read -r -a no_repo_message <<< $(get_tmux_option "@penumbra-git-no-repo-message" "")
IFS=' ' read -r -a no_untracked_files <<< $(get_tmux_option "@penumbra-git-no-untracked-files" "false")
...
# getting the #{pane_current_path} from penumbra.sh is no longer possible
getPaneDir()
{
 nextone="false"
